#!/usr/bin/env python
"""Espera a que terminen los jobs HyP3 'choique-int{tag}' y descarga+descomprime a products_int{tag}/.

    python watch_download.py --tag 80
    python watch_download.py --tag 80
"""
...
def main() -> None:
    ...
    hyp3 = sdk.HyP3()
    jobs = hyp3.find_jobs(name=f"choique-int{args.tag}")
    print(f"jobs choique-int{args.tag}: {len(jobs)}")
    if not args.no_wait:

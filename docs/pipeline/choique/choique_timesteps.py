#!/usr/bin/env python
"""Bajo del Choique — análisis detallado: voidage de reservorio por pozo (elipse orientada al
lateral) vs subsidencia acumulada InSAR, en varios timesteps + panel temporal del bloque.

Universo: los 41 pozos del bloque. 29 tienen TRAYECTORIA (elipse orientada al lateral, en el
centroide de la rama); los 12 restantes van
...
Panel inferior (análisis detallado del bloque):
  ...
  Eje derecho    = subsidencia mediana sobre los pozos [mm].
    ...
    ~/miniforge3/bin/mamba run -n insar python exploraciones/choique/choique_timesteps.py
"""
...
DATA = HERE / "_data"
TS = HERE / "mintpy_int80" / "timeseries_ERA5_ramp_demErr.h5"
NPANELS = 6
MARGIN_PX = 12
...
def main() -> None:
    wells = json.load(open(DATA / "choique_wells_all.json"))
    mon = pd.read_csv(DATA / "choique_monthly_perwell.csv").sort_values("ym")
    ...
    # ventana: solo pozos con trayectoria o con volumen (los acuíferos/parados
    # remotos estiran el encuadre y dejan los pads en una esquina)
    activos = set(mon[mon.v_tot + mon.v_inj > 0].idpozo.astype(str)) \
        | {idp for idp, w in wells.items() if w["has_traj"]}
    with h5py.File(TS, "r") as f:
        ...
        allx = np.array([w["cx"] for idp, w in wells.items() if idp in activos])
        ally = np.array([w["cy"] for idp, w in wells.items() if idp in activos])
        ...
        # suavizado temporal gaussiano (σ=45 días, como el slider): acá la señal
        # (~−20 mm) es comparable a la atmósfera de una época suelta
        ts_win = f["timeseries"][:, r0:r1, c0:c1] * 1000.0
        tdec = np.array([(int(d[:4]) + (int(d[4:6]) - 1) / 12 + int(d[6:8]) / 365.25)
                         for d in dates])
        cube = np.empty((len(idx), *ts_win.shape[1:]), "float32")
        for k, i in enumerate(idx):
            wts = np.exp(-0.5 * ((tdec - tdec[i]) / (45.0 / 365.25)) ** 2)
            cube[k] = np.tensordot(wts / wts.sum(), ts_win, axes=(0, 0))
            # re-referencia local: resta la mediana espacial del cuadro (modo común
            # atmosférico/estacional ±10 mm que acá compite con la señal)
            cube[k] -= np.nanmedian(cube[k])
        ...
        # mediana sobre pozos MENOS mediana de la ventana (misma re-referencia local)
        fondo = np.nanmedian(ts_win, axis=(1, 2))
        sub_t = np.nanmedian(crop[:, ly, lx], axis=1) * 1000.0 - fondo
        # versión suavizada (σ=45 días) para la curva del panel inferior
        sub_t_s = np.array([
            np.average(sub_t, weights=np.exp(-0.5 * ((tdec - ti) / (45.0 / 365.25)) ** 2))
            for ti in tdec])
    ...
    for k, (ax, d) in enumerate(zip(map_axes, sel_dates)):
        ...
        im = ax.imshow(cube[k], extent=ext, origin="upper", cmap="RdYlBu",
                       norm=TwoSlopeNorm(0.0, vmin, max(vmax, 0.5)), aspect="equal")
        cum = cum_at(ym)
        for idp, w in wells.items():
    ...
    fig.colorbar(im, ax=map_axes, shrink=0.55, location="right", pad=0.01,
                 label="Despl. LOS acumulado relativo al entorno local [mm]  (− = subsidencia)")
    ...
    ax2.plot(td, sub_t, color="#b2182b", lw=0.7, alpha=0.35)
    ax2.plot(td, sub_t_s, color="#b2182b", lw=2.2,
             label="subsidencia mediana de los pozos (suav. 45 d)")
    ax2.set_ylabel("Desplazamiento LOS mediano [mm]", color="#b2182b")
    ax2.tick_params(axis="y", labelcolor="#b2182b")
    ax2.legend(loc="lower left", fontsize=8)
    axd.set_title("Análisis detallado Bajo del Choique — voidage acumulado del bloque (41 pozos) vs subsidencia",
                  fontsize=11)

    fig.suptitle("Bajo del Choique — voidage por pozo (elipse=trayectoria, círculo=solo boca) "
                 f"vs subsidencia InSAR  ·  ref {dates[0][:4]}-{dates[0][4:6]}", fontsize=14)
    out = HERE / "choique_timesteps.png"

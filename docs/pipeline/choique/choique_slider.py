#!/usr/bin/env python
"""Bajo del Choique — deformación acumulada en el tiempo (slider) + voidage por pozo que crece
...
Cada paso del slider muestra, hasta esa fecha:
  ...
  - elipses por pozo (orientadas al lateral): anillo = voidage de reservorio total acumulado,
    ...
    ~/miniforge3/bin/mamba run -n insar python exploraciones/choique/choique_slider.py
"""
...
def write_html(frames, bounds, vmax, wells, ct_max):
    ...
    if CONC.exists():
        ...
        feats = [f for f in gj["features"]
                 if "CHOIQUE" in (json.dumps(f.get("properties", {})).upper())]
        conc = json.dumps({"type": "FeatureCollection", "features": feats}) if feats else "null"
    html = f"""<!DOCTYPE html><html lang="es"><head><meta charset="utf-8">
<title>Bajo del Choique — deformación + voidage acumulado (slider)</title>
...
<div class="panel">
 <div><button id="play">▶</button> &nbsp;<b>Bajo del Choique</b> · <span id="date"></span></div>

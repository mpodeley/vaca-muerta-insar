#!/usr/bin/env python
"""La Calera — mapa de velocidad LOS + desplazamiento acumulado del bloque.
...
Salidas: calera_velocity.png + _data/calera_velocity_mm_yr.tif

    ~/miniforge3/bin/mamba run -n insar python exploraciones/calera/mapas_velocidad.py
...
def main() -> None:
    ...
    fig.suptitle("La Calera — subsidencia InSAR (track 18 ASC) y pozos", fontsize=13.5)
    fig.tight_layout(rect=[0, 0, 1, 0.95])
    out_png = HERE / f"calera_velocity{args.suffix}.png"
    ...
    out_tif = DATA / f"calera_velocity_mm_yr{args.suffix}.tif"

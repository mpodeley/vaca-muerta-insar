#!/usr/bin/env python
"""La Calera — detección de discontinuidades del campo de velocidad InSAR.
...
Salidas: calera_gradiente.png (4 paneles + rosa de azimuts) y _data/grad_candidates.geojson
(con azimut, |∇v| mediano, largo km por lineamiento).

Por defecto lee ./mintpy_int40 (40 m). Con --src se puede apuntar a otra corrida
MintPy (p.ej. el 80 m de t18_f1050 para un screening previo):

    ~/miniforge3/bin/mamba run -n insar python exploraciones/calera/gradiente.py
...
def main() -> None:
    ...
    fig.suptitle(f"La Calera — discontinuidades del campo de velocidad "
                 f"(paso {step_m:.0f} m, umbral p{args.pctl:.0f} = {thr:.1f} mm/año/km)",
                 fontsize=13)
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    out_png = HERE / f"calera_gradiente{args.suffix}.png"

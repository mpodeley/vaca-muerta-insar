#!/usr/bin/env python
"""La Calera — deformación acumulada en el tiempo (slider) + voidage por pozo que crece
...
Cada paso del slider muestra, hasta esa fecha:
  ...
  - elipses por pozo (orientadas al lateral): anillo = voidage de reservorio total acumulado,
    ...
    ~/miniforge3/bin/mamba run -n insar python exploraciones/calera/calera_slider.py
"""
...
DATA = HERE / "_data"
TS = HERE / "mintpy_int40" / "timeseries_ERA5_ramp_demErr.h5"
MASK = HERE / "mintpy_int40" / "maskTempCoh.h5"
CONC = _PROJECTS / "activos/estado-del-sistema/public/data/concesiones_neuquina.geojson"
OUT = HERE / "demo_calera_slider.html"
MARGIN_PX = 16
...
def main():
    ...
    wells = json.load(open(DATA / "calera_wells_all.json"))
    mon = pd.read_csv(DATA / "calera_monthly_perwell.csv").sort_values("ym")
...
def write_html(frames, bounds, vmax, wells, ct_max):
    ...
    if CONC.exists():
        ...
        feats = [f for f in gj["features"]
                 if "LA CALERA" in (json.dumps(f.get("properties", {})).upper())]
        conc = json.dumps({"type": "FeatureCollection", "features": feats}) if feats else "null"
    html = f"""<!DOCTYPE html><html lang="es"><head><meta charset="utf-8">
<title>La Calera — deformación + voidage acumulado (slider)</title>
...
<div class="panel">
 <div><button id="play">▶</button> &nbsp;<b>La Calera</b> · <span id="date"></span></div>
